...
import { CrDeleteClusterLambdaProps } from "./lambda";
...
export function addListECSTaskRole(
  ...
  policy.addStatements(
    ...
    new iam.PolicyStatement({
      resources: [`arn:aws:ecs:*:*:task/${ecsClusterName}/*`],
...
export function addAccessMskPolicies(
  ...
  policy.addStatements(
    new iam.PolicyStatement({
      resources: [`arn:aws:kafka:*:*:cluster/${mskClusterName}/*`],
      actions: ["kafka-cluster:Connect", "kafka-cluster:DescribeCluster"],
    }),

    new iam.PolicyStatement({
      resources: [`arn:aws:kafka:*:*:topic/${mskClusterName}/*/*`],
      actions: [
    ...
    new iam.PolicyStatement({
      resources: [`arn:aws:kafka:*:*:group/${mskClusterName}/*/*`],
...
export function addPoliciesToCrCreateS3SinkConnectorLambda(
  ...
  policy.addStatements(
    new iam.PolicyStatement({
      resources: [
        `arn:aws:kafkaconnect:*:*:connector/${clusterName}-s3-sink-connector/*`,
        `arn:aws:kafkaconnect:*:*:custom-plugin/${clusterName}-connector-s3-plugin/*`,
      ],
      actions: [
  ...
  policy.addStatements(
    new iam.PolicyStatement({
      resources: [`arn:aws:s3:::${logS3BucketName}`],
      actions: [
...
export function addPoliciesToCrGetMskConfigVersionLambda(
  ...
  policy.addStatements(
    new iam.PolicyStatement({
      resources: [`arn:aws:kafka:*:*:configuration/${configName}/*`],
...
export function addPoliciesToCrDeleteClusterLambda(
  ...
  policy.addStatements(
    new iam.PolicyStatement({
      resources: [
        `arn:aws:ecs:*:*:container-instance/${custerName}/*`,
        `arn:aws:ecs:*:*:task/${custerName}/*`,
        `arn:aws:ecs:*:*:cluster/${custerName}`,
        `arn:aws:ecs:*:*:service/${custerName}/${serviceName}`,
      ],
      actions: [
        "ecs:UpdateService",
        "ecs:DeleteService",
...
export function grantKinesisStreamReadWrite(
  ...
  policy.addStatements(
    new iam.PolicyStatement({
      resources: [
        `arn:aws:kinesis:*:*:stream/${streamName}`,
        `arn:aws:kinesis:*:*:stream/${streamName}/*`,
      ],
      actions: [

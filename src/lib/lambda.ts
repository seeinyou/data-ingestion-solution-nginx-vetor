  ...
  const fn = new lambda.Function(
    ...
    "cr-create-msk-s3-sink-connector-lambda",
    {
      ...
      code: lambda.Code.fromAsset(
        path.join(__dirname, "./lambda/cr/create-msk-s3-sink-connector/")
      ),
      handler: "app.handler",
      memorySize: 512,
...
export function createCrMskTopicLambda(
  ...
  const fn = new lambda.Function(scope, "cr-create-msk-topic-lambda", {
    runtime: lambda.Runtime.PYTHON_3_9,
    code: lambda.Code.fromAsset(
      path.join(__dirname, "./lambda/cr/create-msk-topic/")
    ),
    handler: "app.handler",
    memorySize: 512,
...
export function createAlbReceiverLambda(
  ...
  const fn = new lambda.Function(scope, "alb-receiver-lambda", {
    ...
    code: lambda.Code.fromAsset(
      path.join(__dirname, "./lambda/alb-receiver/target/alb-receiver-lambda.zip")
    ),
    handler: "com.amazon.aws.gcr.csdc.clickstream.server.ALBReceiverLambda::handleRequest",

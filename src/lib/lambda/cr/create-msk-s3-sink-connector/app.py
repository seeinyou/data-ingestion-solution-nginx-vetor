...
#connect_plugin_url = 'https://github.com/lensesio/stream-reactor/releases/download/4.0.0/kafka-connect-aws-s3-kafka-2-8-4.0.0.zip'
connect_plugin_url = 'https://d1i4a15mxbxib1.cloudfront.net/api/plugins/confluentinc/kafka-connect-s3/versions/10.2.2/confluentinc-kafka-connect-s3-10.2.2.zip'
...
def handler(event, context):
    ...
    connector_name = f"{msk_cluster_name}-s3-sink-connector"
    plugin_name = f"{msk_cluster_name}-connector-s3-plugin"
...
def create_s3_sink_plugin(plugin_name):
    ...
    plugin_bucket_arn = "arn:aws:s3:::{}".format(plugin_s3_bucket)
...
def getConnectorConfiguration():
    if 'lensesio/stream-reactor' in connect_plugin_url: 
        # this plugin only works in us-east-1
        # https://github.com/lensesio/stream-reactor/blob/master/kafka-connect-aws-s3/README-sink.md
        sink_file_format = 'json'  # JSON, Avro, Parquet, Text, CSV_WithHeaders, CSV
        partition_by = 'PARTITIONBY year,month,day'
        configuration =  {
            "connector.class": "io.lenses.streamreactor.connect.aws.s3.sink.S3SinkConnector",
            "key.converter.schemas.enable": "false",
            "connect.s3.kcql": f"INSERT INTO {sink_s3_bucket}:{sink_s3_obj_prefix} SELECT * FROM {msk_topic} {partition_by} STOREAS `{sink_file_format}` WITH_FLUSH_INTERVAL=60 WITH_FLUSH_COUNT=50000",
    ...
    if 'confluentinc-kafka-connect' in connect_plugin_url: 
